...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( minabs.length, 1, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the minimum absolute value of a one-dimensional ndarray', function test( t ) {
	...
	v = minabs( [ vector( x, 6, 1, 0 ) ] );
	t.strictEqual( v, 0.0, 'returns expected value' );

	x = [ -4.0, -5.0 ];
	v = minabs( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, 4.0, 'returns expected value' );

	x = [ -0.0, 0.0, -0.0 ];
	v = minabs( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( isPositiveZero( v ), true, 'returns expected value' );

	x = [ NaN ];
	v = minabs( [ vector( x, 1, 1, 0 ) ] );
